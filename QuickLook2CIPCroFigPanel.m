function [outpt,k]=QuickLook2CIPCroFigPanel
% this version is used to make the final figure
...
for i=1:length(out)
    ...
    for j=1:length(out(i).data)
        ...
        try
        [ccout]=nlinfit(xtmp,ytmp,@(cc,xx) 1./(1+(xx./cc(1)).^cc(2)),[mean(xtmp),2]);
...
% let's average over the ic50's for the 4 technical reps
for i=1:size(k,1)
...
clusters=statGroupClusters(results,means) 
   writetable(tbl,'AnovaMultCompCIPCRO.csv')
